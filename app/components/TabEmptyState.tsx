...
export default function TabEmptyState({ icon, title, description, variant = "empty" }: TabEmptyStateProps) {
  const iconColor = variant === "error" ? "text-red-400" : variant === "loading" ? "text-gray-500" : "text-emerald-400";
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full text-center px-4">
      <div className="relative mb-6">
        <div className={`w-16 h-16 rounded-2xl bg-gradient-to-br ${bgGradient} flex items-center justify-center border ${borderColor}`}>
          <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className={iconColor}>
            <path d={icon} />
          </svg>
        </div>
        {variant === "loading" && (
      ...
      <h3 className="text-lg font-semibold text-gray-100 mb-2 tracking-tight">{title}</h3>
